...
* paru configuration
Configuring paru to work with [[https://github.com/SqrtMinusOne/sqrt-emacs][my custom Emacs build]].
...
#+begin_src scheme :tangle .config/metapac/groups/remote.toml :noweb yes
<<packages("remote")>>
#+end_src
...
Firejail config:
#+begin_src firejail :tangle .config/firejail/max.profile
...
#+begin_src conf-desktop :tangle .local/share/applications/max.desktop
[Desktop Entry]
Type=Application
Terminal=false
Name=MAX
