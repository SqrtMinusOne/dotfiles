...
#+NAME: mail-username
#+begin_src bash
pass show Digital/Email/pvkorytov@etu.ru | sed -n 's/username: //;2p'
#+end_src
...
# sslcacertfile = /etc/ssl/certs/ca-certificates.crt
cert_fingerprint = AD:10:E1:44:2C:B8:F4:4A:9B:39:39:32:44:75:1E:E1:55:3C:B2:B3:CD:91:27:ED:50:D3:69:EB:C7:AC:2D:8D
#+end_src
* Notmuch
| Arch dependency |
|-----------------|
...
*** Blacklist
Blacklist filter for the =post-new= hook. Reads email addresses from =blacklist.txt= (one per line, =#= for comments) and tags matching new emails as spam.

#+NAME: blacklist-filter
#+begin_src bash :tangle no
BLACKLIST="$HOME/10-19 Code/11 Config/11.11 email-blacklist/blacklist.txt"
if [ -f "$BLACKLIST" ]; then
    while IFS= read -r addr; do
        addr=$(echo "$addr" | sed 's/#.*//' | xargs)
        [ -z "$addr" ] && continue
        notmuch tag +spam -inbox -- "from:$addr AND tag:new"
    done < "$BLACKLIST"
fi
#+end_src
...
passwordeval "pass show Service/google-main-app-password | head -n 1"
...
tls on
tls_starttls off
tls_fingerprint AD:10:E1:44:2C:B8:F4:4A:9B:39:39:32:44:75:1E:E1:55:3C:B2:B3:CD:91:27:ED:50:D3:69:EB:C7:AC:2D:8D
from pvkorytov@etu.ru
user pvkorytov
passwordeval "pass show Digital/Email/pvkorytov@etu.ru | head -n 1"
#+end_src
...
** Blacklist sender
Tags the message as spam, removes inbox/unread, and adds the sender's address to =blacklist.txt=. The search mode variant acts on all selected threads when a region is active.
#+begin_src emacs-lisp
...
Update blacklist:
#+begin_src bash :tangle ~/bin/scripts/blacklist-candidates
EMAILS=$(
    notmuch show --body=false --format=json --entire-thread=false \
            'tag:pvkorytov AND (tag:spam OR subject:[MASSMAIL] OR subject [Spam]) AND NOT from:*@etu.ru' \
        | jq -r '.[][] | .[0].headers.From | capture("<(?<addr>[^>]+)>")?.addr // .' \
        | sort \
        | uniq
)

mapfile -t patterns < "/home/pavel/10-19 Code/11 Config/11.11 email-blacklist/blacklist.txt"

while IFS= read -r email; do
    for pattern in "${patterns[@]}"; do
        [[ -z $pattern || $pattern =~ ^# ]] && continue
        [[ $email == $pattern ]] && continue 2
    done
    echo "$email"
done <<< "$EMAILS"
#+end_src
