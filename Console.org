...
#+end_src
*** Ghostel
Ghostel integration, see the [[https://dakra.github.io/ghostel/#orga5520f8][docs]].
...
#+begin_src bash :tangle ~/.codex/hooks/permission-notify.sh
payload=$(cat)

active_window=$(xdotool getactivewindow 2>/dev/null || true)
codex_window=${WINDOWID:-}

# Notify unless we can positively determine that Codex's terminal is focused.
if [[ -z "$codex_window" || "$active_window" != "$codex_window" ]]; then
    description=$(
        jq -r '
    ...
    # Keep the action listener alive after the hook itself returns.  An absolute
    # icon path avoids Dunst's deliberately restricted icon search path.
    (
        action=$(notify-send \
            --app-name="Codex" \
            --urgency=normal \
            --icon=/usr/share/icons/Papirus-Dark/24x24/actions/dialog-password.svg \
            --action=default="Open Codex terminal" \
            "Codex needs permission" \
            "$description")

        if [[ "$action" == "default" ]]; then
            if [[ -n "$codex_window" ]]; then
                xdotool windowactivate "$codex_window" 2>/dev/null || alacritty
            else
                alacritty
            fi
        fi
...
# Produce no decision: Codex should show its normal approval prompt.
exit 0
#+end_src

Add this to the =~/.codex/config.toml=:
#+begin_src toml
...
type = "command"
command = "~/.codex/hooks/permission-notify.sh"
timeout = 5
statusMessage = "Sending permission notification"
#+end_src

After that, run =/hooks= and confirm the hook.
* SSH
I guess there are no secrets here.

#+begin_src conf-space :tangle ~/.ssh/config
# TODO get CIDR
# ip -brief -4 address
#  ip -4 route show proto kernel scope link
# Match originalhost *.digital.etu.ru localnetwork 10.24.32.0/20
#     ProxyJump none

Match originalhost *.etu.ru,10.136.*,10.128.*,10.160.*
	ProxyJump pavel@192.168.122.50
...
#+end_src
** =process-cue=
#+begin_src sh :tangle bin/scripts/process-cue
FLAC_NAME="${1%.cue}.flac"
FOLDER="${1%.cue}-split"
...
cuebreakpoints "$1" | shnsplit -o flac "$FLAC_NAME"
cuetag.sh "$1" split-*.flac
mkdir -p "$FOLDER"
mv split-*.flac "$FOLDER"
#+end_src
* Arch settings
#+NAME: packages
#+begin_src emacs-lisp :tangle no
