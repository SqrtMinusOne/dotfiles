...
=company-mode= gets slow when automatic completion probes a remote filesystem.
Keep manual completion, but disable the idle timer in remote file and shell
buffers.
#+begin_src emacs-lisp
...
Somehow there isn't a built-in function?
