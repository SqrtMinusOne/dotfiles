...
| org-transclusion         | 0ab00dec9f76a55110f153a766806c6820341443 |
| calfw                    | 0ab00dec9f76a55110f153a766806c6820341443 |
| org-timeblock            | 0ab00dec9f76a55110f153a766806c6820341443 |
| elfeed                   | 3c1593651761c57462c030e077b761ce7f815304 |
| wallabag                 | 3c1593651761c57462c030e077b761ce7f815304 |
...
#+end_src
** compat
Somehow Emacs provides a fake =compat.el=? WTF?
...
Also, why is the wrong transient loaded?
#+begin_src emacs-lisp
...
I've actually mostly switched to =uv= and =nvm=, so don't know if I should keep that...
...
:header-args:emacs-lisp: :tangle /home/pavel/.emacs.d/modules/sqrt-maintenance.el :comments links
:END:
[[https://github.com/alberti42/straight-overview][straight-overview]] is a package to perform selective upgrades with straight.el.
...
The 2020 Nerd Font build predates the supplementary private-use glyphs used by current tools.  Preserve its existing glyphs and use Symbols Nerd Font Mono only for ranges that the old font does not contain.
...
#+end_src
*** node-debug
[[https://github.com/SqrtMinusOne/node-debug][node-debug]] is my AI-generated package to debug node.js.
...
Integrate with EXWM and perspective.el:
#+begin_src emacs-lisp
...
*** rest-query
[[https://github.com/SqrtMinusOne/rest-query/][rest-query]] is my AI-generated package to poke REST APIs.
...
** Databases
:PROPERTIES:
:MODULE_NAME: db-programming
:header-args:emacs-lisp: :tangle /home/pavel/.emacs.d/modules/sqrt-db-programming.el :comments links
:END:
[[https://github.com/emarsden/pgmacs][pgmacs]] is PostgreSQL client for Emacs.
...
#+end_src
*** Restore double-prefix tangle for Org 10
Org 10.0-pre currently computes the =C-u C-u C-c C-v t= target filter from the raw =:tangle= header value, but compares it with normalized tangle target file names. The result is that double-prefix tangling can report zero blocks even when the block under point has a valid =:tangle= target.
...
I'll need more priorities for tasks. A-J will work, I think:
#+begin_src emacs-lisp
(setq org-priority-highest ?A)
(setq org-priority-lowest (+ ?A 9))
#+end_src

Then, effort estimation. Effort per task can be set with =M-x org-set-effort=, which picks values from the list below:
#+begin_src emacs-lisp
(with-eval-after-load 'org
  (add-to-list
...
But I also occasionally have tasks I work for a long time. For that, I want to specify how much time I plan to spend on on the task a particular day for further workflows.

I've decided to store that information in blocks like this:
#+begin_example
:EFFORT:
<2026-06-08 Mon>: 04:00
:END:
#+end_example
The timestamp is active because that way it's indexed in =org-agenda=.

So, first, parse the bounds of this block:
#+begin_src emacs-lisp
...
Which allows us to implement interactive functions to add and remove daily durations:
#+begin_src emacs-lisp
...
***** View backlog
Lately I've been keeping a lot of tasks in the TODO state, moving the deadlines ever forward. This is probably suboptimal, so I'll try to keep these tasks in the TODO state, without a deadline but with a priority, and use =org-ql= and maybe =org-agenda= to get a list of such tasks.
...
***** Work around org-store-link regression
#+begin_src emacs-lisp
...
[[https://github.com/ml729/org-habit-stats][org-habit-stats]] is a pretty nice package.
...
#+end_src
**** Custom agendas
Custom agendas to fit with my workflow.

***** Exclude tags from agenda
I want to filter some tags. See [[https://emacs.stackexchange.com/questions/18179/org-agenda-command-with-org-agenda-filter-by-tag-not-working][this answer]] at Emacs StackExchange for filtering the =agenda= block by tag:
#+begin_src emacs-lisp
...
***** Display effort and clocked time in day heading
I also want to integrate =org-clock= and org effort estimations with org-agenda. Essentially, I want to display three pieces of information:
- Total planned effort for this day
- Total clocked time in this day
- The number of tasks without effort estimation, if any.

The used extension point for this is =org-agenda-format-date=.

So, first, planned effort. Get the effort of task:
#+begin_src emacs-lisp
...
Get clocked time at point. I think it's find to do one by one because =org-ql= caches actions:
#+begin_src emacs-lisp
...
And get the data for a particular timestamp.
#+begin_src emacs-lisp
...
I couldn't find a better way to display the missed tasks than to just return them in the function above. So here's the display function:
#+begin_src emacs-lisp
...
And, finally, the custom date format function. For some reason, =org-mode= overrrides all faces applied here, so I do custom spans which are later converted to faces.
#+begin_src emacs-lisp
...
***** Display effort and clocked time in task heading
I also want to display effort and clocked time in task heading.

There's =org-agenda-prefix-format=, which can be used for that, but it doesn't allow for many complex calculations, so I use it to reserve a placeholder:
#+begin_src emacs-lisp
...
Then a function to get the prefix string:
#+begin_src emacs-lisp
...
Get all agenda files and archive:
#+begin_src emacs-lisp
...
Metapac stores the same info in TOML like this:

#+begin_src toml :exports none
flatpak = {
  repos = [
    {
      name = "user:flathub",
...
Also, some package repositories require additional parameters in the group file, e.g. =flatpak= requires to specify repositories.
...
[[https://github.com/jwiegley/emacs-async/][dired-async]] allows makes the built-in dired operations asynchronous by spawning child emacs instances. it's useful when =rsync= isn't available, which, in my case, is tramp via webdav.
...
*** bookmarks
Config for the built-in =bookmark.el= package.
...
For SSH connections, enable =tramp-direct-async-process=:
#+begin_src emacs-lisp
...
Cleanup FUSE on exit:
#+begin_src emacs-lisp
(setq tramp-fuse-unmount-on-cleanup t)
#+end_src

It looks like there's a bug in =auth-source= with TRAMP recursively entering the same connection? I don't know.
#+begin_src emacs-lisp
...
=wakatime-mode= and =activity-watch-mode= start subprocesses from the current directory.  In a remote buffer that means extra TRAMP processes (including remote Git and curl calls for ActivityWatch), so keep both trackers local.
#+begin_src emacs-lisp
...
=company-mode= gets slow when automatic completion probes a remote filesystem. Keep manual completion, but disable the idle timer in remote file and shell buffers.
#+begin_src emacs-lisp
...
*** Cleanup
A function to cleanup a TRAMP connection.
...
A function to maybe do so, or kill the current buffer.
#+begin_src emacs-lisp
...
*** Copy one file over SFTP
Unfortunately I couldn't find a good way to copy files via TRAMP over SSH other than to launch =scp= myself. =dired-rsync= doesn't work without remote =rsync=; =dired-async= is opaque and doesn't show progress.
...
:END:
*** ghostel
[[https://github.com/dakra/ghostel][ghostel]] is yet another terminal emulator for Emacs, probably better than all the rest. See the [[https://dakra.github.io/ghostel/#ghostel-vs-vterm][feature comparison]].

I keep configs for other emulations for now.
...
**** Dedicated buffer
Make a dedicated buffer for ghostel in the bottom of the screen.
...
One reason I wanted this was to have a "normal" (...evil...) editing experience for shell prompts. My current shell setup is fish with vi bindings, which doesn't work well with vterm because the shell just accepts the keystrokes from the terminal, completely oblivious to the current Emacs state. So, I need to do stuff like entering insert state in Emacs, then entering normal state in the shell while keeping Emacs in insert state. That's just too much pain sometimes.

This setup also frequently messes with fish autosuggestions.

Also, I wanted to be able to run =dired= or =find-file= from the terminal. I've sort of implemented that in the [[*vterm][vterm]] section, but an Emacs-integrated shell is obviously more convenient for that.

UPD <2026-08-14 Fri>: =ghostel= does this acceptably well.
...
I keep this because =eat= can integrate with =eshell=.
#+begin_src emacs-lisp
(use-package eat
...
:PROPERTIES:
:MODULE_NAME: el-rss
:header-args:emacs-lisp: :tangle /home/pavel/.emacs.d/modules/sqrt-el-rss.el :comments links
:END:
[[https://github.com/SqrtMinusOne/el-rss][el-rss]] is my AI-generated client for [[https://tt-rss.org/][tt-rss]] and [[https://readeck.org/en/][Readeck]].

I used to use [[https://github.com/emacs-elfeed/elfeed][elfeed]], but I gradually switched to tt-rss, mainly because I like to read RSS from mobile. Check out my old blog posts on how I used elfeed:
- [[https://sqrtminusone.xyz/posts/2021-09-07-emms/][My EMMS and elfeed setup]] (2021)
- [[https://sqrtminusone.xyz/posts/2022-05-09-pdf/][Extending elfeed with PDF viewer and subtitles fetcher]] (2022)
- [[https://sqrtminusone.xyz/posts/2022-09-16-vosk/][Podcast transcripts with elfeed & speech recognition engine]] (2022)
I'll eventually write a post about my current workflow.
...
*** LaTeX and pandoc
I sometimes find that I'm not a fan of reading articles from Emacs. Somehow what works for program code doesn't work that well for natural text. The best solution I've found so far is to render selected articles as PDFs. I may even print out some large articles I want to read.
...
The code for the template is available [[file:.emacs.d/rdrview.tex][in the dotfiles repo]]. If you use LaTeX, you'll probably be better off using your own setup. The template consumes the standard =title=, =subtitle=, =author=, and =date= metadata, plus =main-lang= and =other-lang= variables for polyglossia.
...
And invoke pandoc. The renderer writes into a temporary PDF beside the destination and renames it only after Pandoc exits successfully.
...
**** Opening el-rss articles
Now we have everything required to render =el-rss= articles. I've asked AI to add =el-rss-ui-current-article= that returns a data structure similar to elfeed's, and a function to export the current article with its resources into a folder.
...
Now =g v= opens the cached PDF from either an el-rss article or article-list buffer. The first render may take a moment; subsequent opens reuse the PDF.
...
I use it occasionally to open links from el-rss.
...
#+end_src
**** Custom modeline and online status
Setting up the modeline. The default mode string doesn't look great with my other modeline modules, so I override that.

Also, I want to display unread messages from chats and groups separately, so here are some custom functions:
#+begin_src emacs-lisp
...
#+end_src
**** Fix regressions
Arch's TDLib 1.8.65 changed =inputMessagePhoto=: the photo fields moved under an =inputPhoto= object.  Keep telega's older photo attachments working when that new TDLib API is installed.
#+begin_src emacs-lisp
...
TDLib can briefly hand telega private chats whose user object has no status yet. Do not let that break root buffer rendering.
#+begin_src emacs-lisp
...
The same missing-type issue can happen inside the chat status inserter itself. Catch only that exact nil =:@type= case and fall back to the last message.
#+begin_src emacs-lisp
...
:PROPERTIES:
:MODULE_NAME: rocket
:header-args:emacs-lisp: :tangle /home/pavel/.emacs.d/modules/sqrt-rocket.el :comments links
:END:
[[https://github.com/SqrtMinusOne/rocket.el][rocket.el]] is my AI-generated Rocket.Chat client.
...
:PROPERTIES:
:MODULE_NAME: max
:header-args:emacs-lisp: :tangle /home/pavel/.emacs.d/modules/sqrt-max.el :comments links
:END:
max.el is my AI-generated MAX client.
...
A function to create a screenshot and save it to a temporary directory:
#+begin_src emacs-lisp
...
A function to attach a screenshot to a notmuch buffer:
#+begin_src emacs-lisp
...
A function to attach a screenshot to org:
#+begin_src emacs-lisp
...
:END:
AI workflows and tooling goes here.
...
There are a lot of built-in commands, none of which I found useful. But I've made use of the package for some of my workflows.
...
So first, I need to diff two strings. The built-in diff isn't so great, so I prefer difftastic.
#+begin_src emacs-lisp
...
Edit <2026-08-12 Wed>: Replaced the unmaintained wrapper with my AI-generated [[https://github.com/SqrtMinusOne/whisper-cli][whisper-cli]], based on =faster-whisper= and optional pyannote diarization.

**** Whisper
[[https://github.com/openai/whisper][OpenAI Whisper]] is an amazing speech recognition toolkit.

I previously used [[https://github.com/ggerganov/whisper.cpp][whisper.cpp]] and [[https://github.com/Vaibhavs10/insanely-fast-whisper][insanely-fast-whisper]]. Now I use my own CLI around [[https://github.com/SYSTRAN/faster-whisper][faster-whisper]], with GPU acceleration, direct SRT/VTT/TXT output, and optional speaker diarization through [[https://github.com/pyannote/pyannote-audio][pyannote.audio]].
...
The CLI creates the final SRT, VTT, and TXT files, so Emacs only has to start it asynchronously and report completion. Hugging Face credentials are passed through the child environment only when diarization is requested.
...
If run interactively, the defined function prompts for paths to both files. Diarization can be disabled, run with automatic speaker counting, or given an exact speaker count.
...
This section used to be an elaborate elfeed integration, but I don't use elfeed anymore, so I'm fine with running the package on random URLs.
...
Run with =M-x subed-mpv-play-from-file=.
...
*** AI Agents
**** agent-shell
Running AI agents from Emacs. It's pretty nice but I'll wait when plan mode is available in =codex-acp=.

Install =ai-proxy= and =codex-acp=:
#+begin_src bash
...
Fix =RET=. Why is it always broken?
#+begin_src emacs-lisp
...
*** timestamps
Some utilities for interactive UNIX timestamp manipulation.
#+begin_src emacs-lisp
