...
***** Integrate weekly view with org-clock
...
The used extension point for this is =org-agenda-format-date= (there's also =org-agenda-prefix-format= for individual tasks, which I'll use later).
...
I want the fragments to be clickable:
#+begin_src emacs-lisp
...
And, finally, the custom date format function:
#+begin_src emacs-lisp
