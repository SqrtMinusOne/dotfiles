...
| Type | Note                    |
|------+-------------------------|
| FILE | [[file:.config/flameshot/flameshot.ini##template][flameshot.ini##template]] |

[[https://github.com/flameshot-org/flameshot][flameshot]] is my program of choice to make screenshots.

As it overwrites its own config all the time, I template the config file with yadm and keep the template in git. Nothing really interesting in the linked file except changed UI color.
* dunst
| Type | Note                            |
|------+---------------------------------|
