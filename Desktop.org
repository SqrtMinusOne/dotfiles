...
x-scheme-handler/http=glide-browser-bin.desktop
x-scheme-handler/https=glide-browser-bin.desktop
    ...
    ((error)
     (exwm--log "Error occurred while running command hooks: %s\n\nBacktrace:\n\n%s"
                ...
                (with-temp-buffer
                  ...
                  (buffer-string))))))
#+end_src
*** Floating windows and mouse warp
Prevent mouse cursor from jumping to the first monitor when a floating window appears.

I guess this happens because =exwm-floating--set-floating='s call to =select-frame-set-input-focus= occurs before the frame's xcb position requests are flushed, so =focus-follows-mouse= and =mouse-autoselect-window= warp the pointer using stale off-screen coordinates, which get clamped to (0,0) by the X server.
...
#+end_src
*** Floating windows resize fix
When a floating X client resizes itself (e.g. a dialog expanding), EXWM's ConfigureRequest handler only resizes the Emacs frame's outer window but not the frame container, so the container clips the enlarged frame. This advice queries the outer window's new geometry and applies it to the container as well.
...
#+end_src
** Background
I want to have a background image in EXWM. Emacs doesn't really support this, so the below only works on my [[https://github.com/SqrtMinusOne/sqrt-emacs][custom build]] that adds the =alpha-background= window parameter.

Anyway, first we need to set the background image:
#+begin_src emacs-lisp
(defun my/exwm-set-wallpaper ()
  (call-process-shell-command "feh --bg-fill ~/Pictures/wallpaper.jpg"))
#+end_src

Then, we can make a major mode for =initial-major-mode= to set the window parameter. The parameter will have to be reset on the buffer leave.
...
Also excluding modules that are not available on certain hosts:
        ...
        (seq-map
         (lambda (hostname)
           (seq-map
            (lambda (el) (nth 1 el))
            (seq-filter
             (lambda (el) (and (string-equal (nth 0 el) hostname)
                               (nth 1 el)))
             exclude-table)))
         `(,@hostnames "")))
...
- =hostnames= is a list of unique hostnames in =exclude-table=
- =exclude-combinations= is a list of lists of module names to be excluded for each hostname
- =module-glyphs-combinations= is a list of lists of actual modules for each hostname
- =color-changes= is a list of unique adjacent colors across modules on all hosts
...
To configure polybar itself, we need to generate a set of modules for each host.

The parameters here, excluding the two required tables, are:
- =hostname= - the current hostname on which to filter out the blocks by the =polybar_modules_exclude= table,
- =first-color= - the first color of the first glyph,
- =last-color= - the second color of the last glyph.

#+NAME: polybar-generate-modules
#+begin_src emacs-lisp :var table=polybar_modules exclude-table=polybar_modules_exclude hostname="default" first-color="background" last-color="background"
(let* ((exclude-modules
        (thread-last
          exclude-table
          (seq-filter (lambda (el) (string-equal (nth 0 el) hostname)))
...
Fonts
#+begin_src conf-windows
font-0 = pango:DejaVu Sans:size=13;2
...
- have different blocks and settings on each host;
...
declare -A BLOCKS=(
    ["azure"]="<<polybar-generate-modules(hostname="azure")>>"
    ["eminence"]="<<polybar-generate-modules(hostname="eminence")>>"
    ["iris"]="<<polybar-generate-modules(hostname="iris")>>"
    ["indigo"]="<<polybar-generate-modules(hostname="indigo")>>"
    ["violet"]="<<polybar-generate-modules(hostname="violet")>>"
    ["weiss"]="<<polybar-generate-modules(hostname="weiss")>>"
    ["default"]="<<polybar-generate-modules(hostname="default")>>"
)
...
nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader | sed -s 's/ %/%/;s/,/  /g'
#+end_src
...
I use [[https://github.com/naelstrof/maim][maim]] for selecting the region and [[https://github.com/gabm/Satty][satty]] for annotation and copying.

#+begin_src bash :tangle ~/bin/scripts/screenshot :shebang "#!/usr/bin/env bash"
output_filename="${1:-$HOME/Pictures/screenshots/satty-$(date '+%Y%m%d-%H:%M:%S').png}"
...
* Glide
:PROPERTIES:
:header-args+: :tangle ./.config/glide/glide.ts
:END:

[[https://glide-browser.app/][Glide]] is a keyboard-driven fork of Firefox.

| Category | Arch dependency        |
|----------+------------------------|
| browsers | glide-browser-bin      |
...
** Theme
Glide exposes the [[https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/theme/update][browser.theme.update()]] API directly in the config file, so we can just use that:

#+begin_src typescript :noweb yes
async function updateTheme() {
  await browser.theme.update({
    colors: {
      frame: <<get-color(name="bg", quote=1)>>,
      tab_background_text: <<get-color(name="fg", quote=1)>>,
      toolbar: <<get-color(name="modeline", quote=1)>>,
      toolbar_text: <<get-fg-for-color(name="modeline", quote=1)>>,
      toolbar_field: <<get-color(name="bg", quote=1)>>,
      toolbar_field_text: <<get-color(name="blue", quote=1)>>,
      tab_line: <<get-color(name="yellow", quote=1)>>,
      popup: <<get-color(name="bg-alt", quote=1)>>,
      popup_text: <<get-color(name="fg", quote=1)>>,
      tab_loading: <<get-color(name="yellow", quote=1)>>,
    },
  })
}
...
#+begin_src conf-desktop :tangle .local/share/applications/glide-browser-bin.desktop
[Desktop Entry]
Name=Glide Browser
...
| browsers  | torbrowser-launcher    |
** Office & Multimedia
| Category | Arch dependency             | Disabled | Source  |
|----------+-----------------------------+----------+---------|
...
| office   | okular                      |          |         |
| office   | obs-studio                  |          |         |
| office   | user:chat.rocket.RocketChat |          | flatpak |
| office   | digikam                     |          |         |
| office   | zotero-bin                  |          |         |
| office   | openshot                    | t        |         |
...
| office   | max-bin                     |          |         |
** Geo
| Category | Arch dependency        | Source  | Disabled |
|----------+------------------------+---------+----------|
| geo      | user:org.qgis.qgis     | flatpak | t        |
| geo      | user:org.viking.Viking | flatpak |          |
...
Geo
#+begin_src scheme :tangle .config/metapac/groups/geo.toml :noweb yes
<<packages("geo")>>
#+end_src
...
Games
#+begin_src scheme :tangle .config/metapac/groups/games.toml :noweb yes
<<packages("games")>>
#+end_src
** Flatpak
...
** polkit-agent
#+begin_src conf :tangle .config/systemd/user/polkit-agent.service
[Unit]
...
#+end_src
** opensnitch-ui
#+begin_src conf :tangle .config/systemd/user/opensnitch-ui.service
[Unit]
Description=OpenSnitch UI

[Service]
ExecStart=/usr/bin/opensnitch-ui

[Install]
WantedBy=wm-session.target
#+end_src
** snixembed
[[https://git.sr.ht/~steef/snixembed][snixembed]] is a compatibility proxy between two systemtray protocols.

| Arch dependency |
|-----------------|
| snixembed       |

#+begin_src conf :tangle .config/systemd/user/snixembed.service
[Unit]
